...
(async (): Promise<void> => {
  const { wallet, number, asset } = parse<BulkMintScriptArgs>({
    asset: {
      type: String,
      alias: 'a',
  ...
  let tokenAddress = '';
  if (asset === 'car') {
    bulkMintMax = env.bulkCarMintMax;
    tokenId = parseInt(env.tokenCarId, 10);
    tokenAddress = env.tokenCarAddress;
  } else if (asset === 'driver') {
    bulkMintMax = env.bulkDriverMintMax;
    tokenId = parseInt(env.tokenDriverId, 10);
    tokenAddress = env.tokenDriverAddress;
  } else {
    bulkMintMax = env.bulkFuelMintMax;
    tokenId = parseInt(env.tokenFuelId, 10);
    tokenAddress = env.tokenFuelAddress;
  }

  console.log(bulkMintMax, tokenId, tokenAddress);

  if (number >= Number(bulkMintMax))
